...
class AVLST<Key extends Comparable<Key>, Value> {
	private class Node{
		Key key;
		Value value;
		Node left, right;
		...
		public Node(Key k, Value v, int c, int h) {
			key = k;
			value = v;
			count = c;
			height = h;
	...
	private Node delete(Node node, Key key) {
		...
		if (cmp>0) {
			...
			if (node.left==null) {
				...
				Key minInRightKey = min(node.right);
				Value minInRightValue = get(minInRightKey);
				node.key = minInRightKey;
				node.value = minInRightValue;
				node.right = delete(node.right, key);
			}
		}
		node.count = 1+size(node.left)+size(node.right);
		node.height = 1+Math.max(size(node.left), size(node.right));
	...
	private Node rightRotate(Node node) {
		...
		node.count = size(node.left)+size(node.right)+1;
		node.height = 1+Math.max(size(node.left), size(node.right));
		newNode.count = size(newNode.left)+size(newNode.right)+1;
		newNode.height = 1+Math.max(size(newNode.left), size(newNode.right));
		return newNode;
	}
	
	private Key min(Node node) {
		if (node==null) return null; //this should never happen 
...
public class AVLTree {
	...
	public static void main(String[] args) {
		AVLST<Integer, Integer> tree = new AVLST<>();
//		AVLTreeST<Integer, Integer> tree = new AVLTreeST<>();
		...
		tree.put(43, 1);
		tree.inorder();
//		Iterable<Integer> inorder = tree.keysInOrder();
//		for(int u: inorder) {
//			System.out.print(u+" ");
//		}
//		System.out.println();
